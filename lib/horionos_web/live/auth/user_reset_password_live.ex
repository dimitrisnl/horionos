defmodule HorionosWeb.Auth.UserResetPasswordLive do
  ...
  def handle_event("reset_password", %{"user" => user_params}, socket) do
    case RateLimiter.check_rate("reset_password:#{socket.assigns.user.id}", 5, 3_600_000) do
      {:ok} ->
        case PasswordReset.reset_password(socket.assigns.user, user_params) do
          {:ok, _} ->
  ...
  defp assign_user_and_token(socket, %{"token" => token}) do
    if user = PasswordReset.validate_reset_token(token) do
